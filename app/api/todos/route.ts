...
export async function GET(nextRequest: Request) {
  ...
  if (flag) {
    flag = false;
    const completedTodoAfterOneDay = todos.filter(
      (each) =>
        each.completed &&
        new Date().getTime() - new Date(each.updateTime ?? "").getTime() >
          24 * 60 * 60 * 1000,
    );
    ...
    if (completedTodoAfterOneDay.length > 0) {
      try {
        ...
        const remainingTodos = parsedTodosData.todos.filter(
          (each: any) =>
            !completedTodoAfterOneDay.some(
              (completedEach) => completedEach.id === each.id,
            ),
        );
        const updatedTodosData = { ...parsedTodosData, todos: remainingTodos };
        const parsedDeletedTodosData = JSON.parse(deletedTodosData);
        const updatedDeletedTodosData = {
          ...parsedDeletedTodosData,
          todos: [...parsedDeletedTodosData.todos, ...compressedTodos],
        };
        try {
          await Promise.all([
            writeFile(
...
export async function POST(nextRequest: Request) {
  ...
  if (!body?.title) {
    return new Response(
      JSON.stringify({ status: false, message: "Title is required" }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    );
  }
  console.log(process.cwd());
  try {
    const exisitingData = await readFile(
      `${process.cwd()}/app/api/todos/testData.json`,
      "utf-8",
    );
    ...
    const updatedParsedData = {
      ...parsedData,
      todos: [...parsedData.todos, newTodos],
    };
    try {
      await writeFile(
        "app/api/todos/testData.json",
        JSON.stringify(updatedParsedData, null, 2),
        "utf-8",
      );
    } catch (err) {
