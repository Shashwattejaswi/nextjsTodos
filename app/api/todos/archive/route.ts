...
{
    try{
        const readDeletedTodos =await readFile(`${process.cwd()}/app/api/todos/deletedTodos.json`, "utf-8");
        const {todos} = JSON.parse(readDeletedTodos);
        if(todos.length > 0)
        {
            const unCompressedTodos = todos.map((each:any)=> {

                const {updateTime,...rest}=JSON.parse(zlib.gunzipSync(Buffer.from(each.compressedTodo,"base64")).toString())
               return{ id:each.id,
                ...rest,
                deletedTime:each.deletedStamp
            }
