...
const AddPopup = ({handleUpdateTodo, setShowAddPopup,updateBeforeTodos }: {handleUpdateTodo: (todosText:string) => void, setShowAddPopup: (show: boolean) => void ,updateBeforeTodos?:any}) =>{

    
    const [todosText, setTodosText] = useState<string>("");

    useEffect(()=>{
        if(updateBeforeTodos)
        setTodosText(updateBeforeTodos.title)
    },[updateBeforeTodos])
    ...
    return <form onSubmit={handleSubmit} onReset={handleReset} className="flex items-center justify-center fixed top-0 left-0 w-full h-full bg-[#00000066] p-4 rounded-lg shadow-lg">
       <div className="w-1/3 bg-white p-4 rounded-md">
         <h2 className="text-xl font-bold mb-4">{updateBeforeTodos?"Update ":"Add New "} Todos</h2>
        ...
        <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="cursor-pointer px-4 py-2 bg-blue-500 text-white rounded">{updateBeforeTodos?"Update":"Add"}</button>
        </div>
       </div>
    </form>
}
